...
var DEFAULT_POS = { zoom: 1, fx: 0.5, fy: 0.5 };
...
function renderSheetToCanvas(sheet, snaps) {
  ...
  const pool = (snaps && snaps.length > 0) ? snaps : (window.DEMO_SNAPS || []);
  const sourceFor = (g) => {
    if (sourceMap[g] != null) return sourceMap[g];
    const pick = pool.length ? pool[g % pool.length] : null;
    if (!pick) return null;
